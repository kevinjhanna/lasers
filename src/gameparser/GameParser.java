...
public class GameParser {
	...
	public Game parse() throws IOException, InvalidLoadedBoardException{
		...
		try {
			...
			while (stream.hasNextLine()) {
					String aLine = stream.nextLine().replaceAll("(#.*|\\s)", ""); // dejamos de lado los comentarios
					if (!aLine.matches("^\\s*$")){
	...
	private void processTile(String aLine, Map<Position, Tile> tiles) throws InvalidLoadedBoardException{
		...
		Tile realTile = null;
		Direction direction = Direction.NORTH;// TODO pasar un int de rotation a un Direction
